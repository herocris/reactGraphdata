...
import { setWeaponConfiscations, onSetActiveWeaponConfiscation, onAddNewWeaponConfiscation, onUpdateWeaponConfiscation, onDeleteWeaponConfiscation, onSetTableOptions, onLoading, onSetErrorMessage } from '../slices';
...
export const startSaveWeaponConfiscation = (weaponConfiscation: WeaponConfiscation) => {
    return async (dispatch: Dispatch) => {
        dispatch(onLoading(true));
        dispatch(onSetActiveWeaponConfiscation({ ...weaponConfiscation, foto: '' }))
        try {
            ...
            dispatch(onAddNewWeaponConfiscation({ id, ...resto }))
        }
        catch (error: any) {
            console.log(error);
            return handleApiError(error, dispatch);
...
export const startUpdateWeaponConfiscation = (weaponConfiscation: WeaponConfiscation) => {
    return async (dispatch: Dispatch) => {
        ...
        if (weaponConfiscation.foto instanceof File) {
            formData.append('logo', weaponConfiscation.foto);
        }
...
export const startDeleteWeaponConfiscation = (weaponConfiscation: WeaponConfiscation) => {
    return async (dispatch: Dispatch) => {
        dispatch(onLoading(true));
        dispatch(onSetActiveWeaponConfiscation(weaponConfiscation))
        try {
            await calendarApi.delete(`/weaponConfiscation/${weaponConfiscation.identificador}`,);
            dispatch(onDeleteWeaponConfiscation())
        } catch (error) {
            return handleApiError(error, dispatch);
        } finally {
            dispatch(onLoading(false));
