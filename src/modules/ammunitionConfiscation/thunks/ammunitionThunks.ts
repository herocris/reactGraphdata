...
import { setAmmunitionConfiscations, onSetActiveAmmunitionConfiscation, onAddNewAmmunitionConfiscation, onUpdateAmmunitionConfiscation, onDeleteAmmunitionConfiscation, onSetTableOptions, onLoading, onSetErrorMessage } from '../slices';
...
export const startSaveAmmunitionConfiscation = (ammunitionConfiscation: AmmunitionConfiscation) => {
    return async (dispatch: Dispatch) => {
        dispatch(onLoading(true));
        dispatch(onSetActiveAmmunitionConfiscation({ ...ammunitionConfiscation, foto: '' }))
        try {
            ...
            dispatch(onAddNewAmmunitionConfiscation({ id, ...resto }))
        }
        catch (error: any) {
            console.log(error);
            return handleApiError(error, dispatch);
...
export const startUpdateAmmunitionConfiscation = (ammunitionConfiscation: AmmunitionConfiscation) => {
    return async (dispatch: Dispatch) => {
        ...
        if (ammunitionConfiscation.foto instanceof File) {
            formData.append('logo', ammunitionConfiscation.foto);
        }
...
export const startDeleteAmmunitionConfiscation = (ammunitionConfiscation: AmmunitionConfiscation) => {
    return async (dispatch: Dispatch) => {
        dispatch(onLoading(true));
        dispatch(onSetActiveAmmunitionConfiscation(ammunitionConfiscation))
        try {
            await calendarApi.delete(`/ammunitionConfiscation/${ammunitionConfiscation.identificador}`,);
            dispatch(onDeleteAmmunitionConfiscation())
        } catch (error) {
            return handleApiError(error, dispatch);
        } finally {
            dispatch(onLoading(false));
